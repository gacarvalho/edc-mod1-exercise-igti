...
def pipeline_enem():
	...
	def emr_process_enem_data():
		cluster_id = client.run_job_flow(
			Name='EMR-SESSON-Airflow',
			ServiceRole='EMR_DefaultRole',
			JobFlowRole='EMR_EC2_DefaultRole',
			VisibleToAllUsers=True,
			LogUri='s3://datalake-sesson-edc-tf/emr-logs/',
			...
			Instances={
				...
				'Ec2KeyName': 'iam-gacarvalho-key-pair',
				'KeepJobFlowAliveWhenNoSteps': True,
				'TerminationProtected': False,
				'Ec2SubnetId': 'subnet-083c1179855ee1412'
			},
			...
			Steps=[{
				...
				'HadoopJarStep': {
					...
					'Args': ['spark-submit',
							...
							'--master', 'yarn',
							'--deploy-mode', 'cluster',
							's3://datalake-sesson-edc-tf/emr-code/pyspark/01_delta_spark_insert.py'
						]
				}
			}],
		)
				...
				Steps=[{
					...
					'HadoopJarStep': {
						...
						'Args': ['spark-submit',
								...
								'--master', 'yarn',
								'--deploy-mode', 'cluster',
								's3://datalake-sesson-edc-tf/emr-code/pyspark/02_delta_spark_upsert.py'
							]
					}
				}]
			)
